...
class EnvironmentAgent : Agent
{
    ...
    private const int MaxPriceToSellToUtility = 10; //max possible price to sell 1kWh to the utility company (in pence)
...

    {
        switch (message.Content)
        {
            case "start": //this agent only responds to "start" messages
                ...
                int generation = rand.Next(MinGeneration, MaxGeneration); //for the first experiment
